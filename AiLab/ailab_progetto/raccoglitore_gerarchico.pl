...
two_level_planner:add_del(0, va_a_pulire(P2), St, [in(P2),batteria(B2),capacita(C2), raccolto(R2)], [in(P1),batteria(B1),capacita(C1), raccolto(R1) | Sporco], Cost) :-
	% la capacita deve essere maggiore di  zero
	member(capacita(C1),St),
	C1 > 0,
	% parto da P1 e vado allo sporco in P2
	member(in(P1), St),
	member(sporco(P2),St),
	...
	% RULE 3: non vi deve essere altro punto da pulire tra P1 e P2
	foreach(member(sporco(P),St),(P == P2; not(point_between(P,P1,P2)))),
	%
	get_action_plan([in(P1),sporco(P2)], va_a_pulire(P2), _Plan, Cost),
	member(batteria(B1),St),
	B2 is B1 - Cost,
	B2 >= 0,
	member(capacita(C1),St),
	C2 is C1 - 1,
	(sa(P2,sporco_ostinato) -> Sporco = []; Sporco = [sporco(P2)]),
...
two_level_planner:add_del(0, va_a_caricare, St, [in(P2),batteria(B2),fine], [in(P1),batteria(B1)], Cost) :-
...
two_level_planner:h(0, St, H) :-
	...
	h1(St,X),
	H is B1 + (B2 + 1)* (
		B2 - R + (1/(C2 + 1))*(1 + C2 - C1 + (B2 - B1 + X)/(B2 - B1 + X + 1))
	).
...
two_level_planner:starting_state(_,
	...
	findall(sporco(P), sporco(P,_Qty), Sporco),
	in(In),
	base(Base),
	cestino(Cestino),
	batteria(Batteria),
